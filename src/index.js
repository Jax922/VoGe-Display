    ...
    case bodySegmentation.SupportedModels.MediaPipeSelfieSegmentation: {
      ...
      if (runtime === 'mediapipe') {
        return bodySegmentation.createSegmenter(STATE.model, {
          ...
          modelType: STATE.modelConfig.type,
          solutionPath:
            `https://cdn.jsdelivr.net/npm/@mediapipe/selfie_segmentation@${mpSelfieSegmentation.VERSION}`
        });
      } else if (runtime === 'tfjs') {
        return bodySegmentation.createSegmenter(STATE.model, {
          runtime,
...
async function renderChart(uid, slide) {
  try {
      const data = await getData(uid, slide);
...
async function app() {
  ...
  // render video and body segmentation 
  // renderPrediction();
  // document.getElementById('video').srcObj = camera;


  // get data from firebase, and render chart
  renderChart(urlParams.get('userId'), urlParams.get('slide'));

  // handle gesture
  // gestureHandleMain();
  // window.requestAnimationFrame(gestureHandleMain);
